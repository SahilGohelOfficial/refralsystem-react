...
const ChainColumn = ({ chain, onUserClick, headerExtra, dimmed = false }: ChainColumnProps) => {
  ...
  return (
    <div className="w-64 shrink-0 glass-card flex flex-col">
      <div className="px-4 py-3.5 border-b border-border bg-surface/40">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <h3 className={`text-sm font-semibold truncate ${dimmed ? 'text-text-secondary' : 'text-text'}`}>
              {chain.name}
            </h3>
      ...
      <div
        className={`p-3 space-y-2 flex-1 overflow-y-auto max-h-[calc(100vh-280px)] ${dimmed ? 'opacity-60' : ''}`}
      >
        {chain.users.length === 0 ? (
